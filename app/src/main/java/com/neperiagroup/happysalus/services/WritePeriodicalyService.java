...
public class WritePeriodicalyService extends IntentService {
    ...
    private List<SaveDataBean> calorieToday;
    ...
    protected void onHandleIntent(Intent i) {
        ...
        while (active) {
            Log.e(TAG, "start while service background");
            ...
            try {
                ...
                if(mPerformerListener.getDeepSleep()!=0 || mPerformerListener.getLightSleep()!=0){
                    ...
                    Gson gson = new Gson();
                    String json = gson.toJson(sleepSaveDataBean);
                    storeInMemory.setValueStringToStore("lastSleep",json);
                }

                if(mPerformerListener.getCalories()!=0){
                    toSave = new SaveDataBean();
                    toSave.setDate(now);
                    toSave.setValue(String.valueOf(mPerformerListener.getCalories()));
                    calorieToday.add(toSave);
                    Gson gson = new Gson();
                    String json = gson.toJson(toSave);
                    storeInMemory.setValueStringToStore("lastCalorie",json);
                }
                if(mPerformerListener.getDistance()!=0){
                    toSave = new SaveDataBean();
                    toSave.setDate(now);
                    toSave.setValue(String.valueOf(mPerformerListener.getDistance()));
                    distanceToday.add(toSave);
                    Gson gson = new Gson();
                    String json = gson.toJson(toSave);
                    storeInMemory.setValueStringToStore("lastDistance",json);
                }

                writeToMemory();
                mPerformerListener.clearAll();

                Log.e(TAG, "stop while service background");
    ...
    private void readTodayData() {
        ...
        try {
            ...
            if (!json.equals("")) {
                calorieToday = new Gson().fromJson(json, listTypeDaySaveDataBean);
            } else {
                calorieToday = new ArrayList<>();
            }
        ...
        try{
            Type listTypeDaySaveDataBeanPress = new TypeToken<SleepSaveDataBean>() {
            }.getType();
    ...
    private void writeToMemory() {
        ...
        if(sleepSaveDataBeanToday!=null) {
            json = gson.toJson(sleepSaveDataBeanToday);
            Log.i("---string---sleep", json);
            try {
