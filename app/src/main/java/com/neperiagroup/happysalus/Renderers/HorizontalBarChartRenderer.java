...
public class HorizontalBarChartRenderer extends BarChartRenderer {
    ...
    protected void drawDataSet(Canvas c, IBarDataSet dataSet, int index) {
        ...
        for (int j = 0; j < buffer.size(); j += 4) {
            ...
            {
                changeBitmapColor(imageToRender[0],Color.WHITE);
            }
            if(x10<(right-starOffset))
            {
                changeBitmapColor(imageToRender[1],Color.WHITE);
            }
            if(x20<(right-starOffset))
            {
                changeBitmapColor(imageToRender[2],Color.WHITE);
            }
            if(x30<(right-starOffset))
            {
                changeBitmapColor(imageToRender[3],Color.WHITE);
            }
            if(x40<(right-starOffset))
            {
                changeBitmapColor(imageToRender[4],Color.WHITE);
            }
            if(x50<(right-starOffset))
            {
                changeBitmapColor(imageToRender[5],Color.WHITE);
            }
            drawImage(c,imageToRender[0],x0+22,top-15);
            drawImage(c,imageToRender[1],x10+10,top-15);
            drawImage(c,imageToRender[2],x20+10,top-15);
            drawImage(c,imageToRender[3],x30+10,top-15);
            drawImage(c,imageToRender[4],x40+10,top-15);
            drawImage(c,imageToRender[5],x50-22,top-15);
        }
    }
