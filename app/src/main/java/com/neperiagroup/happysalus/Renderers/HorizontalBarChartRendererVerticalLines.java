...
public class HorizontalBarChartRendererVerticalLines extends BarChartRenderer {

    private final Bitmap[] imageToRender;
    public HorizontalBarChartRendererVerticalLines(BarDataProvider chart, ChartAnimator animator,
                                      ViewPortHandler viewPortHandler, Bitmap[] image) {
    ...
    protected void drawDataSet(Canvas c, IBarDataSet dataSet, int index) {
        ...
        for (int j = 0; j < buffer.size(); j += 4) {
            ...
            int starWidth = imageToRender[0].getWidth();
            int starOffset = starWidth / 2;

            //float maxLength = mBarShadowRectBuffer.right-starOffset;

            float x0 = left-starOffset;
            float x10 = ((mBarShadowRectBuffer.right-starOffset)/5);
            float x20 = ((mBarShadowRectBuffer.right-starOffset)/5)*2;
            float x30 = ((mBarShadowRectBuffer.right-starOffset)/5)*3;
            float x40 = ((mBarShadowRectBuffer.right-starOffset)/5)*4;
            float x50 = (mBarShadowRectBuffer.right)-starOffset;

            if(x0<(right-starOffset))
            {
                changeBitmapColor(imageToRender[0],Color.TRANSPARENT);
            }
            if(x10<(right-starOffset))
            {
                changeBitmapColor(imageToRender[1],Color.TRANSPARENT);
            }
            if(x20<(right-starOffset))
            {
                changeBitmapColor(imageToRender[2],Color.TRANSPARENT);
            }
            if(x30<(right-starOffset))
            {
                changeBitmapColor(imageToRender[3],Color.TRANSPARENT);
            }
            if(x40<(right-starOffset))
            {
                changeBitmapColor(imageToRender[4],Color.TRANSPARENT);
            }
            if(x50<(right-starOffset))
            {
                changeBitmapColor(imageToRender[5],Color.TRANSPARENT);
            }
            //drawImage(c,imageToRender[0],x0,top);
            drawImage(c,imageToRender[1],x10,top);
            drawImage(c,imageToRender[2],x20,top);
            drawImage(c,imageToRender[3],x30,top);
            drawImage(c,imageToRender[4],x40,top);
            //drawImage(c,imageToRender[5],x50,top);
        }
    }
