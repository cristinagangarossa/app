...
public class DashboardActivity extends AppCompatActivity{
    ...
    protected void onCreate(Bundle savedInstanceState){
        ...
        //int isConnect = getIntent().getIntExtra("flagIsConnect", 0);
        if(storeInMemory.getListNameBtcDeviceUserBindFromStore(user.getId()).size()!=0){// && !isUserBound()){//Se ci sono dispositivi che sono stati associati e l'utente non è ancora connesso
        ...
        pager.addOnPageChangeListener(new ViewPager.OnPageChangeListener(){
            ...
            public void onPageSelected(int position){
                ...
                boolean isPaired = storeInMemory.checkNameBtcDeviceUserBindFromStore(user.getId(), device.getNameBtnDevice());
            ...
            public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {
                switch (deviceService.getDevices().get(selectedDevice).getListDataDevice().get(i).getDataType()){
                    ...
                    case "pressure":
                        ...
                        break;
                    case "balance":
                        startActivity(ScaleStatsActivity.getInstanceIntent(getApplicationContext(),deviceService.getDevices().get(selectedDevice).getListDataDevice().get(i)  ,deviceService.getDevices().get(selectedDevice).getModelDevice()));
                        break;
        ...
        boolean isPaired = storeInMemory.checkNameBtcDeviceUserBindFromStore(user.getId(), deviceService.getDevices().get(0).getNameBtnDevice());
        btnPair.setVisibility((isPaired) ? View.INVISIBLE : View.VISIBLE);
        btnPair.setOnClickListener(new View.OnClickListener(){
            ...
            public void onClick(View view){
                switch(selectedDevice){
                    case 0://Braccialetto
                        startActivity(DeviceFoundActivity.getIstanceIntent(getApplicationContext()));
                        break;
                    case 1://Bilancia
    ...
    public static Intent getInstanceIntent(Context context){
        ...
        //intent.putExtra("flagIsConnect", flagIsConnect);
        return intent;
    }
                    ...
                    public void run() {
                        ...
                        for(ScanDeviceWrapper scanDeviceWrapper : mDevices){//Scorre la lista dei device trovati
                            if(scanDeviceWrapper.getDevice().getName()!=null) {
                                ...
                                List<String> ListNameBtcDeviceUserBind = storeInMemory.getListNameBtcDeviceUserBindFromStore(user.getId());
                                for(String nameBtcDevice : ListNameBtcDeviceUserBind){//Scorre la lista dei device già associati
                                    if (scanDeviceWrapper.getDevice().getName().startsWith(nameBtcDevice)) {//Se corrisponde con il device trovato nello scan
